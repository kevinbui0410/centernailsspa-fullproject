...
const seedAppointments = async () => {
    try {
        ...
        const staffMembers = await User_1.User.find({ role: 'staff' });
        ...
        for (let day = 0; day < 30; day++) {
            const currentDate = (0, date_fns_1.addDays)(startDate, day);
            // Skip weekends
            ...
            for (let i = 0; i < appointmentsPerDay; i++) {
                ...
                const appointmentTime = (0, date_fns_1.setMinutes)((0, date_fns_1.setHours)(currentDate, hour), minute);
                const endTime = (0, date_fns_1.addHours)(appointmentTime, 1); // Each appointment lasts 1 hour
                ...
                appointments.push({
                    ...
                    endTime: endTime,
                    status,
                    notes: `Appointment for ${service.name} with ${(staff === null || staff === void 0 ? void 0 : staff.firstName) || 'Unknown'} ${(staff === null || staff === void 0 ? void 0 : staff.lastName) || ''}`,
                    price: service.price
                });
            }
        }
        // Validate appointments before inserting
        const validAppointments = appointments.filter(app => app.staff &&
            app.service &&
            app.customer &&
            app.startTime &&
            app.endTime);
        ...
        await Appointment_1.Appointment.insertMany(validAppointments);
        console.log(`Seeded ${validAppointments.length} appointments successfully`);
        // Verify all appointments have staff
        const unassignedAppointments = await Appointment_1.Appointment.find({ staff: { $exists: false } });
